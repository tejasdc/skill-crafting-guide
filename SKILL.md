...
**Optional fields:** `disable-model-invocation`, `user-invocable`, `allowed-tools`, `model`, `context`, `agent`, `argument-hint`, `hooks`, `license`, `compatibility`, `metadata` (author, version, mcp-server, category, tags). See `references/yaml-frontmatter.md` for the complete reference.
...
## Step 5b: Advanced Features (Optional)

Most skills only need frontmatter + instructions. These features unlock deeper integration when you need it.

### Dynamic Context Injection

The `` !`command` `` syntax runs shell commands **before** the skill content reaches Claude. The command output replaces the placeholder inline — Claude only sees the result, not the command.
...
When this skill runs, each `` !`command` `` executes immediately, output replaces the placeholder, and Claude receives the fully-rendered prompt with actual data. This is preprocessing — not something Claude executes.
